...
Modified by Owen O'Connor, December 2020
...
PlyFile *ply_write(
    ...
    for (i = 0; i < nelems; i++) {
        ...
        elem->name        = (char *)malloc(sizeof(char) * (strlen(elem_names[i]) + 1));
        strcpy(elem->name, elem_names[i]);
        elem->num    = 0;
        elem->nprops = 0;
    }
...
void ply_put_comment(PlyFile *plyfile, char *comment) {
    ...
    /* add comment to list */
    plyfile->comments[plyfile->num_comments] = (char *)myalloc(sizeof(char) * (strlen(comment) + 1));
    strcpy(plyfile->comments[plyfile->num_comments], comment);
...
void ply_put_obj_info(PlyFile *plyfile, char *obj_info) {
    ...
    /* add info to list */
    plyfile->obj_info[plyfile->num_obj_info] = (char *)myalloc(sizeof(char) * (strlen(obj_info) + 1));
    strcpy(plyfile->obj_info[plyfile->num_obj_info], obj_info);
...
PlyFile *ply_read(FILE *fp, int *nelems, char ***elem_names) {
    ...
    for (i = 0; i < plyfile->nelems; i++) {
        elist[i] = (char *)myalloc(sizeof(char) * (strlen(plyfile->elems[i]->name) + 1));
        strcpy(elist[i], plyfile->elems[i]->name);
    }
...
PlyOtherProp *ply_get_other_properties(
    ...
    printf("Got an element with name %s", elem_name);
    other->name = (char *)myalloc(sizeof(char) * (strlen(elem_name) + 1));
    strcpy(other->name, elem_name);
#if 0
...
PlyOtherElems *ply_get_other_element(
    ...
    other->elem_name = (char *)myalloc(sizeof(char) * (strlen(elem_name) + 1));
    strcpy(other->elem_name, elem_name);
...
void add_element(PlyFile *plyfile, char **words, int nwords) {
    ...
    elem       = (PlyElement *)myalloc(sizeof(PlyElement));
    elem->name = (char *)myalloc(sizeof(char) * (strlen(words[1]) + 1));
    strcpy(elem->name, words[1]);
...
void add_property(PlyFile *plyfile, char **words, int nwords) {
    ...
    if (equal_strings(words[1], "list")) { /* is a list */
        ...
        prop->name           = (char *)myalloc(sizeof(char) * (strlen(words[4]) + 1));
        strcpy((char *)prop->name, words[4]);
        prop->is_list = 1;
    } else { /* not a list */
        prop->external_type = get_prop_type(words[1]);
        prop->name          = (char *)myalloc(sizeof(char) * (strlen(words[2]) + 1));
        strcpy((char *)prop->name, words[2]);
        prop->is_list = 0;
    }
...
void copy_property(PlyProperty *dest, PlyProperty *src) {
    dest->name = (char *)myalloc(sizeof(char) * (strlen(src->name) + 1));
    strcpy((char *)dest->name, src->name);
...
void read_test(char *filename, int *num_faces, int *num_vertices, Vertex ***vlist, Face ***flist) {
    int           i, j, k;
    PlyFile *     ply;
    int           nelems;
    char **       elist;
